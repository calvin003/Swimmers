...
var ROUTES = {
  ...
  heart_rate: {
    sheet: 'HeartChecks',
    headers: ['id', 'name', 'email', 'resting_hr', 'active_hr', 'recovery_seconds', 'created_at', 'received_at'],
    row: function (r) {
      return [r.id || '', r.name || '', r.email || '', r.resting_hr || '', r.active_hr || '', r.recovery_seconds || '', r.created_at || '', new Date()];
    }
  }
};
